...
class Product {
  constructor (productId, productTitle, productPrice) {
...
class Basket {
  ...
  add (product, quantity) {
    ...
    this[product.productId] = product;
  ...
  getTotal() {
    //return the total cost of everything in the basket
    const values = Object.values(this);
    let runningTotal = 0;
    values.forEach(function(item) { 
      runningTotal += (item.quantity) * (item.productPrice);
    })
    return runningTotal;
  }
  ...
  console.log(product)
    ...
    orderLineRequest.productId = product.productId;
    orderLineRequest.price = product.productPrice;
    orderLineRequest.quantity = product.quantity;
    orderLineRequest.subTotal = (product.quantity) * 
...
const tv = new Product('0001', '4KTV', 500);
const iphone = new Product('0002', 'iPhone 12', 1000);
const bread = new Product('0003', 'brioche bread', 8);
basket1.add(tv, 1);
basket1.add(iphone, 1);
basket1.add(tv, 4);
basket1.remove(tv, 1);
// console.log(basket1.getTotal());
// console.log(basket1.getOrderLine(tv));
// console.log(basket1.getOrderLine(bread));
console.log(basket1.remove(tv, 9));
console.log(basket1);
